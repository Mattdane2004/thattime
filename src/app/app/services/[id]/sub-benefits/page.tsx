...
import { FieldLabel, Sheet, Toggle } from "@/components/ui";
...
export default function SubBenefitsRoute({ params }: { params: { id: string } }) {
  ...
  if (!offer?.subscription) {
    return (
      <div className="flex h-full flex-col bg-surface">
        ...
        <div className="pt-12 text-center text-[13px] text-muted">Membership not found</div>
      </div>
    );
  }
  ...
  const services = allOffers.filter((candidate) => candidate.type === "service" && candidate.status === "published");
  const classes = allOffers.filter((candidate) => candidate.type === "class" && candidate.status === "published");
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-8">
        <div className="space-y-4 pt-2">
          <div className="rounded-2xl border border-border bg-canvas px-4 py-3.5">
            <div className="text-[14px] font-semibold text-navy">Manage what each tier includes</div>
            <p className="mt-1 text-[12px] leading-snug text-muted">
              Update booking allowances, discounts and member-only access from one place.
            </p>
          </div>
              ...
              <div key={tier.id} className="rounded-2xl border border-border bg-canvas p-4">
                <div className="flex items-start justify-between gap-3 pb-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 truncate text-[17px] font-semibold text-navy">{tier.name}</div>
                    <div className="mt-0.5 text-[12px] text-muted">{tier.price ? `£${tier.price}/${tier.billingPeriod}` : "No price set"}</div>
                  </div>
          ...
          <button
            type="button"
            onClick={() => router.push(`/app/services/${offer.id}/sub-billing`)}
            className="flex w-full items-center gap-3 rounded-2xl border border-border bg-canvas px-4 py-3.5 text-left hover:bg-border/30"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface">
              <Wallet size={15} className="text-secondary" strokeWidth={1.75} />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-[14px] font-semibold text-navy">Edit tier pricing and terms</span>
              <span className="block truncate text-[12px] text-muted">Price, billing period, joining fee, minimum term and cancellation</span>
            </span>
...
function Header({ onBack, sub }: { onBack: () => void; sub?: string }) {
  return (
    <div className="flex h-16 items-center px-5">
      ...
      <span className="ml-1">
        <span className="block text-[17px] font-semibold leading-tight text-navy">Tiers &amp; benefits</span>
