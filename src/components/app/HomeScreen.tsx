...
function RevenueProgressCard({ period }: { period: Period }) {
  const markers = revenueProgress.markers[period];
  return (
    <Link href="/app/hub" className="mx-4 block">
      <div className="rounded-3xl bg-white p-5 shadow-sm">
        <div className="flex items-start justify-between">
        ...
        <div className="mt-2 grid text-[10px] font-semibold text-muted" style={{ gridTemplateColumns: `repeat(${markers.length}, minmax(0, 1fr))` }}>
          {markers.map((m, i) => (
            <span key={m} className={i === 0 ? "text-left" : i === markers.length - 1 ? "text-right" : "text-center"}>
              {m}
            </span>
          ))}
        </div>
...
// `role` is exposed for smoke tests and route wrappers. The default app route
// cannot accept arbitrary props in Next's App Router, so it renders this view
// without a prop and reads the role store.
export function HomeScreen({ role: roleProp }: { role?: AppRole }) {
